...
**RBAC:** Four roles — `super_admin`, `organizer`, `buyer`, `org_staff`. Guards: `Depends(get_current_user)` for auth check, `Depends(require_role(...))` for role check. `super_admin`/`organizer`/`buyer` are rows in `users` (`models.UserRole`); `org_staff` has no `users` row — it's a self-contained JWT from staff login (`security.create_staff_token`, `routers/staff.py`) carrying `organizer_id` + `staff_roles`, never re-validated against the DB per-request (see `security.get_current_user`'s `org_staff` branch). `organizer_id` has a different meaning per role: for `organizer` it's the org they run; for `buyer` and `org_staff` it's the org they registered with / were hired by. A handler that only checks "`organizer_id` is set" (instead of also checking `role`) will incorrectly treat a buyer or staff member as that org's owner — use `security.is_active_organizer(user)` / `is_organizer_owner(user, organizer_id, roles=...)` instead of a bare truthy check on `organizer_id`. A staff member is further scoped to specific events via `StaffEventAssignment` (`orm_models.py`, populated from `routers/staff.py`) — `organizer_id` alone does not grant access to every event of that org for `org_staff`.
