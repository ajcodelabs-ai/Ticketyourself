/**
 * Config dialog for a curved seat row.
 */
...
export default function CurvedRowConfigDialog({
    ...
    const [curve_radius, setCurve] = useState(280);
    const [curve_arc_degrees, setArc] = useState(60);
    ...
    return (
        <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Configurar fila curva</DialogTitle>
                </DialogHeader>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                            ...
                            <Input value={row_label} onChange={(e) => setRowLabel(e.target.value)}
                                   autoFocus data-testid="curved-row-label" />
                        </div>
                        <div className="space-y-1">
                            <Label className="text-xs">Cantidad (1-200)</Label>
                            <Input type="number" min={1} max={200} value={seats_count}
                                   onChange={(e) => setSeats(Math.max(1, Math.min(200, Number(e.target.value))))}
                                   data-testid="curved-row-count" />
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                            <Label className="text-xs">Radio del arco (px)</Label>
                            <Input type="number" min={60} value={curve_radius}
                                   onChange={(e) => setCurve(Math.max(60, Number(e.target.value)))} />
                        </div>
                        <div className="space-y-1">
                            <Label className="text-xs">Ángulo del arco (10-180°)</Label>
                            <Input type="number" min={10} max={180} value={curve_arc_degrees}
                                   onChange={(e) => setArc(Math.max(10, Math.min(180, Number(e.target.value))))} />
                        </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                            <Label className="text-xs">Separación (px)</Label>
                            <Input type="number" min={16} max={64} value={seat_spacing}
                                   onChange={(e) => setSpacing(Math.max(16, Math.min(64, Number(e.target.value))))} />
                        </div>
                        <div className="space-y-1">
                            <Label className="text-xs">Primer asiento</Label>
                            <Input type="number" min={0} value={numbering_start}
                ...
                <DialogFooter>
                    ...
                    <Button
                        onClick={() => onConfirm({
                            row_label, seats_count, curve_radius, curve_arc_degrees, seat_spacing,
